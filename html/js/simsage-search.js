...
let chat_control = {
    ...
    set_ws_connected: function(is_ws_connected) {
        ...
        if (!is_ws_connected) {
            ...
            const err_text = jQuery(".error-text").val();
            if (err_text.indexOf('not connected') === 0) {
                this.error('');
            }
            this.connection_retry_count = 1;
...
jQuery(document).ready(function () {
    simsage.init("", settings, function(isSuccess) {
        if (isSuccess) {
            const urlParams = new URLSearchParams(window.location.search);
            if (urlParams.has("search")) {
                const query_str = urlParams.get("search");
                if (query_str && query_str.trim().length > 0) {
                    simsage.do_search(query_str);
